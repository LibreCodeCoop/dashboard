                  ...
                  <div class="row">
                      ...
                      <div class="col-sm-7">
                          <div class="form-group{{ $errors->has('cpf') ? ' has-danger' : '' }}">
                              <input class="form-control{{ $errors->has('cpf') ? ' is-invalid' : '' }}" name="cpf" id="input-cpf" type="text" placeholder="{{ __('CPF') }}" value="{{ old('cpf', $user->cpf) }}" required />
                              @if ($errors->has('cpf'))
                  ...
                  <div class="row">
                      ...
                      <div class="col-sm-7">
                          <div class="form-group{{ $errors->has('address') ? ' has-danger' : '' }}">
                              <input class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" name="address" id="input-address" type="text" placeholder="{{ __('Address') }}" value="{{ old('address', $user->address) }}" required />
                          ...
                          <div class="form-group{{ $errors->has('customers') ? ' has-danger' : '' }}">
                              <select multiple class="form-control{{ $errors->has('customers') ? ' is-invalid' : '' }} custom-select" name="customers[]" size="3" placeholder="{{ __('Customers') }}">
                                  @foreach($customers as $customer)
                                      <option value="{{$customer->id}}"  {{ in_array($customer->id, old("customers") ?: $user->customers->map(function ($c) { return $c->id; })->toArray()) ? "selected": "" }} >{{ $customer->typeable->social_reason }}</option>
                                  @endforeach
                              </select>
